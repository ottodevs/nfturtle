...
contract NFTurtle is ERC721, Ownable {
    ...
    uint256 public constant maxItemsPurchase = 1;
     ...
     */
    function claimSpecialNFTurtle(
        string memory realPhotoIPFS,
        string memory json,
        uint8 discountInPercent,
        bytes memory _signature
    ) external payable {
        //verify signature
        address wallet = _msgSender();
        address signerOwner = ECDSA.recover(
            keccak256(abi.encode(wallet, realPhotoIPFS, json, discountInPercent)),
            _signature
        );
